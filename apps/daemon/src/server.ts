...
export async function startServer({
  ...
  app.post('/api/test/connection', async (req, res) => {
    ...
    try {
      if (body.mode === 'provider') {
        ...
        const openAiLocalBaseUrl =
          protocol === 'openai' && typeof body.baseUrl === 'string'
            ? validateExternalApiBaseUrl(body.baseUrl)
            : null;
        const allowBlankApiKey = Boolean(openAiLocalBaseUrl?.isLocalNetwork);
        if (
          typeof body.baseUrl !== 'string' ||
          typeof body.apiKey !== 'string' ||
          typeof body.model !== 'string' ||
          !body.baseUrl.trim() ||
          (!body.apiKey.trim() && !allowBlankApiKey) ||
          !body.model.trim()
        ) {
          return sendApiError(
            res,
  ...
  const validateExternalApiBaseUrl = (baseUrl) => {
    ...
    if (!validated.parsed) return validated;
    return {
      ...validated,
      isLocalNetwork: isLocalNetworkApiHost(validated.parsed.hostname),
    };
  };
