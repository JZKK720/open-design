...
export async function validateBaseUrlResolved(
  ...
  if (isTrustedLocalApiHost(hostname)) return sync;
...
async function validateLocalOpenAiModel(
  ...
  if (input.protocol !== 'openai' || !isTrustedLocalApiHost(parsed.hostname)) {
    return null;
  }
  ...
  try {
    response = await fetch(url, {
      method: 'GET',
      headers: input.apiKey ? { authorization: `Bearer ${String(input.apiKey)}` } : {},
      signal,
      redirect: 'error',
    });
  } catch {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const completion = inspectProviderCompletion(
        ...
        data,
        model,
        isTrustedLocalApiHost(validated.parsed.hostname),
      );
      if (completion.kind) {
